...
ct_states <- read_csv('data/clean/ct_apt_states.csv')
ct_counties <- read_csv('data/clean/ct_apt_counties.csv')
ct_all <- read_csv('data/clean/all_ct.csv')

all_years <- unique(ct_all$year)
all_vars <- sort(names(ct_all)[6:ncol(ct_all)])
