...
compressed_data <- read_parquet('clientNotesFinalTwo.parquet')
...
df <- compressed_data %>% 
  ...
  unnest(Insurance) %>% 
  unnest(Discharge) %>% 
  unnest(reportedDemographics) %>% 
  unnest(leadSpatialDemo) %>% 
  unnest(caseManagers) %>% 
  unnest(phq9) %>% 
  unnest(gad7) %>% 
  unnest(appointments)
...
df_summary <- df %>% 
  group_by(State, County, Year, Quarter, Month) %>% 
  summarise(
    Total.Clients = n(),
    Average.Age = mean(Age, na.rm = TRUE),
    Average.Client.Days = mean(Total.Client.Days, na.rm = TRUE),
    Average.Phq9.Score = mean(Phq9.Total.Score, na.rm = TRUE),
    Average.Gad7.Score = mean(Gad7.Total.Score, na.rm = TRUE),
    Total.Appointments = sum(Appointment.Status == "Completed", na.rm = TRUE),
    Total.Video.Sessions = sum(Is.Video.Session == TRUE, na.rm = TRUE),
    Average.Appointment.Duration = mean(Duration, na.rm = TRUE),
    County.Population = mean(County.Total.Population, na.rm = TRUE),
    County.Median.Income = mean(County.Median.Household.Income, na.rm = TRUE),
    County.Median.Density = mean(County.Median.Pop.Density, na.rm = TRUE)
  ) %>% 
  ungroup() %>% 
  arrange(State, County, Year, Quarter, Month)

df_appointments <- df %>% 
  group_by(State, County, Year, Quarter, Month, Appointment.Topic) %>% 
  summarise(
    Total.Appointments = n(),
    Completed.Appointments = sum(Appointment.Status == "Completed", na.rm = TRUE),
    Cancelled.Appointments = sum(Appointment.Status == "Cancelled", na.rm = TRUE),
    No.Show.Appointments = sum(Appointment.Status == "No Show", na.rm = TRUE)
  ) %>% 
  ungroup() %>% 
  arrange(State, County, Year, Quarter, Month, Appointment.Topic)

df_clients <- df %>% 
  group_by(State, County, Year, Quarter, Month, AgeGroup, Gender) %>% 
  summarise(
    Total.Clients = n(),
    Average.Age = mean(Age, na.rm = TRUE),
    Average.Client.Days = mean(Total.Client.Days, na.rm = TRUE)
  ...
  ungroup() %>% 
  arrange(State, County, Year, Quarter, Month, AgeGroup, Gender)
